...
function makeRequest(ip = "203.0.113.51"): any {
  const url = new URL(
    "http://localhost:3000/api/providers/search?q=cardio&zipcode=33914"
  );
  return {
    headers: {
      get: (name: string) =>
...
describe("GET /api/providers/search — rate limiting", () => {
  it("returns 429 after the rate limit is exceeded for a single IP", async () => {
    ...
    expect(rateLimit).toHaveBeenCalledWith("providers-search:203.0.113.51", {
      max: 30,
      windowMs: 60_000,
    });
  });
