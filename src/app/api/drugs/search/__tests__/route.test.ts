...
function makeRequest(q = "ami", ip = "203.0.113.50"): any {
  const url = new URL(`http://localhost:3000/api/drugs/search?q=${q}`);
  return {
    headers: {
      get: (name: string) =>
...
describe("GET /api/drugs/search — rate limiting", () => {
  it("returns 429 after the rate limit is exceeded for a single IP", async () => {
    ...
    expect(rateLimit).toHaveBeenCalledWith("drugs-search:203.0.113.50", {
      max: 30,
      windowMs: 60_000,
    });
  });
