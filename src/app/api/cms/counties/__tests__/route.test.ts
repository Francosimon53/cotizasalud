...
function makeRequest(zip = "33914", ip = "203.0.113.40"): any {
  const url = new URL(`http://localhost:3000/api/cms/counties?zip=${zip}`);
  return {
    headers: {
      get: (name: string) =>
...
describe("GET /api/cms/counties — rate limiting", () => {
  it("returns 429 after the rate limit is exceeded for a single IP", async () => {
    ...
    expect(rateLimit).toHaveBeenCalledWith("cms-counties:203.0.113.40", {
      max: 30,
      windowMs: 60_000,
    });
  });
